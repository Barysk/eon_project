...
class Algorithm:
    ...
    def _update_superchannels(self, time: int) -> float:
        ...
        for sup_chan in self.super_channels:
            # ... check if:
            if (sup_chan.route is None # the channel is unassigned
                    or sup_chan.get_desired_rate(time) > sup_chan.modulation.bitrate * sup_chan.channel_number # the demand is too large OR
