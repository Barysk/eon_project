...
def run_tests(test_case: TestInfo, MOD, output_file="results.csv"):
    ...
    dem_obj = read_connections_from_folder(test_case.dem, 500)

    # Open file in append mode
    with open(output_file, "a") as f:
        # Write net, pat, dem info
        f.write(f"net, {test_case.net}\n")
        f.write(f"pat, {test_case.pat}\n")
        f.write(f"dem, {test_case.dem}\n")
        ...
        first_fit_sg = FirstFitSG(
            ...
            modulations = MOD)
        first_fit_sg.run()
        results["FirstFitSG"] = first_fit_sg.get_overall_performance()
        ...
        exact_fit_ng = ExactFitNG(
            ...
            modulations = MOD)
        exact_fit_ng.run()
        results["ExactFitNG"] = exact_fit_ng.get_overall_performance()
        ...
        exact_fit_sg = ExactFitSG(
            ...
            modulations = MOD)
        exact_fit_sg.run()
        results["ExactFitSG"] = exact_fit_sg.get_overall_performance()
